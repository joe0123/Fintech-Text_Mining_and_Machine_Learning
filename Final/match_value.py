    ...
    def get_value(self, page_dict, df):
        for i in df.index:
            ...
            if df.ix[i, "remote"][:4] != "http":
                self.driver.get("https://fund.cnyes.com/search/?fundName=%E9%87%8E%E6%9D%91&order=displayNameLocal&page="+str(page_dict[df.ix[i, "remote"].replace("?", "類")]))
                self.driver.get(self.driver.find_element_by_link_text(df.ix[i, "remote"].replace("?", "類")).get_attribute("href"))
            else:
...
if __name__ == '__main__':
    ...
    df = match_value(1.5).get_value(page_dict, df)
    #print(df)
    df.drop(columns="remote").to_csv("net_value3.csv", encoding="big5", index=False)
